...
function addTask() {
  ...
  // Check if the entered date is valid and not before the current date
  const currentDate = new Date();
  const datePattern = /^(\d{1,2})-(\d{1,2})-(\d{2})$/;
  if (taskText !== '') {
    const dateMatch = deadline.match(datePattern);
    if (!dateMatch) {
      // Display an alert if the date format is not valid
    ...
    if (isNaN(enteredDate.getTime()) || enteredDate < currentDate) {
      // Display an alert if the date is before the current date
